...
like :class:`~llm_redteam.plugins.registry.Registry` or
:class:`~llm_redteam.plugins.policy_registry.PolicyRegistry`:
...
same way :mod:`~llm_redteam.plugins` decouples a campaign's
choice of ``target: {type: mock}`` from the ``MockTarget`` class.

Nothing here imports :mod:`llm_redteam.plugins` or
:mod:`llm_redteam.adapters`: this module only deals in plugin
