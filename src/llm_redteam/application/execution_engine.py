"""ExecutionEngine: runs attacks against a Target with bounded concurrency.

This is deliberately a separate application service from
:class:`~llm_redteam.application.campaign_orchestrator.CampaignOrchestrator`
so that execution concerns (concurrency, timeouts, retries) can evolve
independently of campaign-level sequencing, and so either can be
tested or reused in isolation.
"""

from __future__ import annotations

import asyncio
import time
from collections.abc import Sequence

from llm_redteam.domain.errors import TargetExecutionError
from llm_redteam.domain.models import Attack, AttackResult, ExecutionContext
from llm_redteam.domain.ports import Target


class ExecutionEngine:
    """Executes a batch of attacks against a single :class:`Target`.

    Concurrency is bounded by a semaphore rather than firing every
    attack at once, since real targets (hosted LLM APIs) enforce rate
    limits. A failed individual execution is captured as an
    ``AttackResult`` with ``error`` set rather than aborting the batch
    — see :class:`llm_redteam.domain.errors.TargetExecutionError`.

    Failed executions (timeouts, target-reported errors) are retried
    with exponential backoff up to ``max_retries`` times before the
    engine gives up and returns the last failure — real targets (hosted
    LLM APIs) fail transiently often enough (rate limits, brief network
    blips) that a single attempt understates what a target actually
    resists.
    """

    def __init__(
        self,
        target: Target,
        concurrency: int = 5,
        timeout_seconds: float = 30.0,
        max_retries: int = 2,
        retry_backoff_seconds: float = 1.0,
    ) -> None:
        self._target = target
        self._concurrency = concurrency
        self._timeout_seconds = timeout_seconds
        self._max_retries = max_retries
        self._retry_backoff_seconds = retry_backoff_seconds

    async def run(self, campaign_name: str, attacks: Sequence[Attack]) -> list[AttackResult]:
        """Execute ``attacks`` and return results in the same order."""
        semaphore = asyncio.Semaphore(self._concurrency)

        async def _execute_once(attack: Attack) -> AttackResult:
            ctx = ExecutionContext(
                campaign_name=campaign_name,
                timeout_seconds=self._timeout_seconds,
            )
            started = time.perf_counter()
            try:
                return await asyncio.wait_for(
                    self._target.execute(ctx, attack), timeout=self._timeout_seconds
                )
            except TargetExecutionError as exc:
                return AttackResult(
                    attack_id=attack.id,
                    target_name=self._target.name,
                    error=str(exc),
                    latency_ms=(time.perf_counter() - started) * 1000,
                )
            except TimeoutError:
                return AttackResult(
                    attack_id=attack.id,
                    target_name=self._target.name,
                    error=f"execution timed out after {self._timeout_seconds}s",
                    latency_ms=(time.perf_counter() - started) * 1000,
                )

        async def _run_one(attack: Attack) -> AttackResult:
            async with semaphore:
                result = await _execute_once(attack)
                attempt = 0
                while not result.succeeded and attempt < self._max_retries:
                    await asyncio.sleep(self._retry_backoff_seconds * (2**attempt))
                    attempt += 1
                    result = await _execute_once(attack)
                return result

        return list(await asyncio.gather(*(_run_one(attack) for attack in attacks)))
