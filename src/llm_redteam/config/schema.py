...
from llm_redteam.domain.models import Severity
...
class PluginSpec(BaseModel):
    ...
    :class:`~llm_redteam.plugins.registry.Registry` (e.g.
...
class VulnerabilityConfig(BaseModel):
    ...
    :class:`~llm_redteam.domain.models.vulnerability.Vulnerability`.

    ``name``/``category`` may be omitted when ``id`` matches a
    :class:`~llm_redteam.domain.vulnerabilities.VulnerabilityDefinition`
    registered in
    :data:`~llm_redteam.domain.vulnerabilities.VULNERABILITY_REGISTRY`
...
class CampaignConfig(BaseModel):
    ...
    :func:`llm_redteam.config.loader.build_campaign_runner`,
