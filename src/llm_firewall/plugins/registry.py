...
A firewall guard runs *every* registered policy against each
inspection, so this stores concrete :class:`Policy` instances (as
opposed to a factory registry that builds exactly one plugin by
